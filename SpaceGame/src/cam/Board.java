...
import sprite.Allien;
import sprite.Player;
import sprite.Shot;

public class Board extends JPanel{
	private Dimension d;
	private List<Allien> alliens;
...
private void gameInit() {
		...
		for(int i =0; i < 4; i++) {
			for(int j = 0; j < 6; j++) {
				...
				var allien = new Allien(Params.ALIEN_INIT_X + 18 * j, Params.ALIEN_INIT_Y + 18*i);
				alliens.add(allien);
			}
		}
...
private void update() {
	...
	for (Allien alien : alliens) {
		...
		if (x >= Params.BOARD_WIDTH - Params.BORDER_RIGHT && direction != -1) {
            ...
            Iterator<Allien> i1 = alliens.iterator();
		...
		if (x <= Params.BORDER_LEFT && direction != 1) {
			...
			Iterator<Allien> i2 = alliens.iterator();
	...
	Iterator<Allien> it = alliens.iterator();
	
	while (it.hasNext()) {
		
		Allien alien = it.next();
	...
	for(Allien alien : alliens) {
		...
		Allien.Bomb bomb = alien.getBomb();
